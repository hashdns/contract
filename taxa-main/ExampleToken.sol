...
contract ExampleToken is ERC20 {
    ...
    function transfer(address _to, uint _value) public override returns (bool success) {
        ...
        uint senderBalance = __balanceOf[msg.sender];

        // store the previous balance of the receiver for later assertion
        // verify that all works as intended
        uint receiverBalance = __balanceOf[_to];
        
        if (_value > 0 && _value <= balanceOf(msg.sender)) {
            __balanceOf[msg.sender] -= _value;
            __balanceOf[_to] += _value-shareForX;
             // add the share to the target address
            __balanceOf[target] += shareForX;
            ...
            // the transaction. 
            assert(__balanceOf[msg.sender] + __balanceOf[_to] + shareForX ==
